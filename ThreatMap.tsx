...
export const ThreatMap: React.FC<ThreatMapProps> = ({ incidents, nodes, isLockdownActive = false }) => {
  ...
  const getSeverityColor = (sev: string) => {
    if (sev === 'critical') return '#f85149'; // red-400
    if (sev === 'high') return '#e3b341';     // orange-400
    if (sev === 'medium') return '#e3b341';   // yellow-400
    return '#34d399';                         // indigo-400
  };
  ...
  return (
    <div
      ...
      className="bg-white dark:bg-[#090c10]/60 backdrop-blur-xl border border-slate-200 dark:border-slate-800/80 rounded-2xl p-5 shadow-xs dark:shadow-xl relative overflow-hidden group hover:border-slate-300 dark:hover:border-slate-700/60 transition duration-300 flex flex-col"
    >
            ...
            <g transform={`translate(${zoomTransform.x}, ${zoomTransform.y}) scale(${zoomTransform.k})`}>
              ...
              <g>
                {projectedContinents.map((points, idx) => (
                  <polygon
                    key={`continent-${idx}`}
                    points={points}
                    className="fill-slate-200/90 dark:fill-[#0d1117]/45 stroke-slate-300 dark:stroke-indigo-950/50 stroke-[1.2px] transition-all duration-300 hover:fill-slate-300 dark:hover:fill-[#161b22]/60"
                    vectorEffect="non-scaling-stroke"
                  />
                ))}
              </g>
              ...
              <g>
                {threatCountries.map(tc => {
                  ...
                  const strokeColor = stats.critical > 0 ? '#da3633' : stats.high > 0 ? '#d29922' : '#10b981';
                  ...
                  return (
                    ...
                    >
                      ...
                      <circle
                        ...
                        fill={hasIncidents ? strokeColor : '#484f58'}
                      />
                    </g>
                  );
                })}
              ...
              <g>
                {nodes.map(n => {
                  ...
                  const color = isOperational ? '#10b981' : '#d29922';
