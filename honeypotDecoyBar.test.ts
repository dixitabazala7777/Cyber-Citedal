...
async function runTests() {
  ...
  suite.describe('HoneypotDecoyBar: Dynamic Status Colors & Threat Thresholds', () => {
    suite.it('Test Case 5: Idle / Low (< 30%) triggers Subdued Cyber Blue (#10b981) & ARMED status', () => {
      const idle0 = getHoneypotDecoyState(0);
      suite.expect(idle0.status).toBe('ARMED');
      suite.expect(idle0.colorHex).toBe('#10b981');
      ...
      suite.expect(idle29.colorHex).toBe('#10b981');
    });

    suite.it('Test Case 6: Moderate (30% - 70%) triggers Warning Amber (#D29922) & ENGAGED status', () => {
      const moderate30 = getHoneypotDecoyState(30);
      suite.expect(moderate30.status).toBe('ENGAGED');
      suite.expect(moderate30.colorHex).toBe('#D29922');
      ...
      suite.expect(moderate70.colorHex).toBe('#D29922');
    });

    suite.it('Test Case 7: Heavy Attack (> 70%) triggers Alert Crimson (#DA3633) pulsing glow & CRITICAL status', () => {
      const heavy71 = getHoneypotDecoyState(70.1);
      suite.expect(heavy71.status).toBe('CRITICAL');
      suite.expect(heavy71.colorHex).toBe('#DA3633');
