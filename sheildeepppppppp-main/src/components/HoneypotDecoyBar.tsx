 ...
 *
 * Requirements:
 * - Idle / Low Activity (< 30%): Subdued Cyber Blue (#0EA5E9) -> "ARMED"
 * - Moderate Engagement (30% - 70%): Warning Amber (#F59E0B) -> "ENGAGED"
 * - Heavy Attack / Breach Attempt (> 70%): Alert Crimson with subtle pulsing animation (#EF4444) -> "CRITICAL"
 */
...
export const HoneypotDecoyBar: React.FC<HoneypotDecoyBarProps> = ({
  ...
  if (variant === 'compact') {
    return (
      <div
        className={`bg-[#050b18]/80 backdrop-blur-md border ${isOffline ? 'border-slate-800' : stateTheme.borderClass} ${isOffline ? '' : stateTheme.cardGlow} rounded-xl p-3 font-mono transition-all duration-300 ${className}`}
      >
  ...
  return (
    <div
      ...
      className={`bg-[#0d0d11]/80 backdrop-blur-md border ${isOffline ? 'border-white/10' : stateTheme.borderClass} ${isOffline ? '' : stateTheme.cardGlow} ring-1 ring-white/5 rounded-2xl p-5 md:p-6 transition-all duration-500 font-mono relative overflow-hidden group hover:border-white/20 ${className}`}
    >
      ...
      <div className="my-6 space-y-2.5 relative z-10">
        ...
        <div className="relative w-full bg-slate-950/90 border border-slate-800 rounded-xl p-1.5 shadow-inner">
          <div className="w-full bg-[#020611] h-6 md:h-7 rounded-lg overflow-hidden relative flex items-center">
