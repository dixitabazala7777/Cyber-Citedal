...
export const BlastRadiusGraph: React.FC<BlastRadiusGraphProps> = ({ nodes: gatewayNodes, onIsolateNode }) => {
  ...
  useEffect(() => {
    ...
    const link = svg.append('g')
      ...
      .attr('stroke', d => {
        ...
        if (blastRadius.has(s as string) && blastRadius.has(t as string)) return '#da3633';
        return d.encrypted ? '#30363d' : '#3a2c0c';
      })
      .attr('stroke-width', d => {
        const s = typeof d.source === 'object' ? (d.source as GraphNode).id : d.source;
        const t = typeof d.target === 'object' ? (d.target as GraphNode).id : d.target;
    ...
    const nodeColor = (d: GraphNode) => {
      if (d.id === breachedNode) return '#c22b28';
      if (blastRadius.has(d.id)) return '#d29922';
      return d.type === 'gateway' ? '#10b981' : d.type === 'database' ? '#10b981' : d.type === 'cache' ? '#34d399' : '#8b949e';
    };
    ...
    node.append('circle')
      ...
      .attr('fill', 'none')
      .attr('stroke', d => d.id === breachedNode ? '#c22b28' : '#d29922')
  ...
  return (
    <div className="bg-[#161b22]/60 border border-slate-800 rounded-xl p-4 mt-4">
